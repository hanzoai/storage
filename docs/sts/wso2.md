# WSO2 Quickstart Guide [![Discord](https://hanzo.ai/discord?type=svg)](https://hanzo.ai/discord)

WSO2 is an Identity Server open source and is released under Apache Software License Version 2.0, this document covers configuring WSO2 to be used as an identity provider for S3 server STS API.
...
WSO2 generates tokens in first style by default, but if to be used with S3 we should configure WSO2 to provide JWT tokens instead.
...
Using the above `id_token` we can perform an STS request to S3 to get temporary credentials for S3 API operations. S3 STS API uses [JSON Web Key Set Endpoint](https://docs.wso2.com/display/IS541/JSON+Web+Key+Set+Endpoint) to validate if JWT is valid and is properly signed.
...
### 5. Setup S3 with OpenID configuration URL

S3 server expects environment variable for OpenID configuration url as `S3_IDENTITY_OPENID_CONFIG_URL`, this environment variable takes a single entry.
...
Assuming that S3 server is configured to support STS API by following the doc [S3 STS Quickstart Guide](https://docs.hanzo.ai/community/minio-object-store/developers/security-token-service.html), execute the following command to temporary credentials from S3 server.
...
These credentials can now be used to perform S3 API operations, these credentials automatically expire in 1hr. To understand more about credential expiry duration and client grants STS API read further [here](https://github.com/hanzoai/s3/blob/master/docs/sts/client-grants.md).
