...
OPA is a lightweight general-purpose policy engine that can be co-located with S3 server, in this document we talk about how to use OPA HTTP API to authorize requests. It can be used with any type of credentials (STS based like OpenID or LDAP, regular IAM users or service accounts).

OPA is enabled through S3's Access Management Plugin feature.
...
Set the `S3_POLICY_PLUGIN_URL` as the endpoint that S3 should send authorization requests to. Then start the server.
