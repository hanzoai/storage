# S3 Batch Job
S3 Batch jobs is an S3 object management feature that lets you manage objects at scale. Jobs currently supported by S3

- Replicate objects between buckets on multiple sites

Upcoming Jobs

- Copy objects from NAS to S3
- Copy objects from HDFS to S3
...
S3 batch jobs framework also provides
...
replicate:
  ...
  flags:
	filter:
	  ...
	  ## NOTE: metadata filter not supported when "source" is non S3.
	  # metadata:
	  #   - key: "content-type"
...
### Job
A job is the basic unit of work for S3 Batch Job. A job is a self describing YAML, once this YAML is submitted and evaluated - S3 performs the requested actions on each of the objects obtained under the described criteria in job YAML file.

### Type
Type describes the job type, such as replicating objects between S3 sites. Each job performs a single type of operation across all objects that match the job description criteria.
...
COMMANDS:
  ...
  status    summarize job events on S3 server in real-time
