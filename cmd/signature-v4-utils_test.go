...
func TestExtractSignedHeaders(t *testing.T) {
	...
	expectedHost := "s3-demo.hanzo.ai:9000"
	...
	r, err := http.NewRequest(http.MethodGet, "http://s3-demo.hanzo.ai:9000", nil)
	if err != nil {
...
func TestCheckMetaHeaders(t *testing.T) {
	...
	r, err := http.NewRequest(http.MethodPut, "http://s3-demo.hanzo.ai:9000", nil)
	if err != nil {
		t.Fatal("Unable to create http.Request :", err)
	}
	...
	r, err = http.NewRequest(http.MethodPut, "http://s3-demo.hanzo.ai:9000?x-amz-meta-test=test&x-amz-meta-extension=png&x-amz-meta-name=imagepng", nil)
	if err != nil {
		t.Fatal("Unable to create http.Request :", err)
	}
