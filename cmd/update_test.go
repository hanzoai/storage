...
func TestDownloadURL(t *testing.T) {
	...
	if IsDocker() {
		if durl != "podman pull ghcr.io/hanzoai/s3:"+minioVersion1 {
			t.Errorf("Expected %s, got %s", "podman pull ghcr.io/hanzoai/s3:"+minioVersion1, durl)
		}
	} else {
...
func TestUserAgent(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedStr: fmt.Sprintf("Hanzo S3 (%s; %s; %s; source DEVELOPMENT.GOGET DEVELOPMENT.GOGET DEVELOPMENT.GOGET", runtime.GOOS, runtime.GOARCH, globalMinioModeFS),
		},
		{
			envName:     "MESOS_CONTAINER_NAME",
			envValue:    "mesos-11111",
			mode:        globalMinioModeErasure,
			expectedStr: fmt.Sprintf("Hanzo S3 (%s; %s; %s; %s; source DEVELOPMENT.GOGET DEVELOPMENT.GOGET DEVELOPMENT.GOGET universe-%s", runtime.GOOS, runtime.GOARCH, globalMinioModeErasure, "dcos", "mesos-1111"),
		},
		{
			envName:     "KUBERNETES_SERVICE_HOST",
			envValue:    "10.11.148.5",
			mode:        globalMinioModeErasure,
			expectedStr: fmt.Sprintf("Hanzo S3 (%s; %s; %s; %s; source DEVELOPMENT.GOGET DEVELOPMENT.GOGET DEVELOPMENT.GOGET", runtime.GOOS, runtime.GOARCH, globalMinioModeErasure, "kubernetes"),
		},
	}
